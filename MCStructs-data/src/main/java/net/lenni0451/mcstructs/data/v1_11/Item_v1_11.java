...
public class Item_v1_11 {
    ...
    public static Item_v1_11 getById(final int itemId) {
        for (Item_v1_11 item : ITEM_LIST) {
            if (item.itemId == itemId) return item;
        }
        return null;
    }

    public static Item_v1_11 getByName(final String name) {
        for (Item_v1_11 item : ITEM_LIST) {
    ...
    private Item_v1_11(final int itemId, final String name, final int maxStackSize, final int maxDamage, final boolean hasSubTypes, final int burnTime, final boolean isPotionIngredient) {
        this.itemId = itemId;
        this.name = name;
        ...
        this.isPotionIngredient = isPotionIngredient;
...
String out = "";
for (Object rawItem : REGISTRY) {
    Item item = (Item) rawItem;
    int id = REGISTRY.getIDForObject(item);
    String name = REGISTRY.getNameForObject(item).toString().split(":")[1];
    out += "public static final Item_v1_11 ";
    out += name;
    out += " = new Item_v1_11(" + id;
    out += ", \"" + name + "\"";
    out += ", " + item.maxStackSize;
    out += ", " + item.maxDamage;
    out += ", " + item.hasSubtypes;
    out += ", " + TileEntityFurnace.getItemBurnTime(new ItemStack(item));
    out += ", " + PotionHelper.isReagent(new ItemStack(item));
    out += ");\n";
}
