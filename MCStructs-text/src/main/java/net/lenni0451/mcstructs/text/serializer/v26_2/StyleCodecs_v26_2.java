...
public class StyleCodecs_v26_2 {
    ...
    public static final MapCodec<Style> MAP_CODEC = MapCodecMerger.mapCodec(
            ...
            ExtraCodecs_v26_2.ARGB_COLOR.mapCodec("shadow_color").optional().defaulted(null), Style::getShadowColor,
    ...
    public static class ClickEventCodec {
        public static final MapCodec<OpenUrlClickEvent> OPEN_URL = MapCodecMerger.mapCodec(
                ExtraCodecs_v26_2.UNTRUSTED_URI.mapCodec("url").required(), OpenUrlClickEvent::asUri,
                ClickEvent::openUrl
        );
        ...
        public static final MapCodec<RunCommandClickEvent> RUN_COMMAND = MapCodecMerger.mapCodec(
                ExtraCodecs_v26_2.CHAT_STRING.mapCodec("command").required(), RunCommandClickEvent::getCommand,
                ClickEvent::runCommand
        );
        public static final MapCodec<SuggestCommandClickEvent> SUGGEST_COMMAND = MapCodecMerger.mapCodec(
                ExtraCodecs_v26_2.CHAT_STRING.mapCodec("command").required(), SuggestCommandClickEvent::getCommand,
        ...
        public static final MapCodec<CustomClickEvent> CUSTOM = MapCodecMerger.mapCodec(
                ...
                NbtConverter_v26_2.INSTANCE.toCodec().mapCodec("payload").required(), CustomClickEvent::getPayload,
                ClickEvent::custom
        );
    ...
    public static class HoverEventCodec {
        public static final MapCodec<TextHoverEvent> TEXT = MapCodecMerger.mapCodec(
                TextCodecs_v26_2.TEXT.mapCodec("value").required(), TextHoverEvent::getText,
                TextHoverEvent::new
        );
        public static final MapCodec<ItemHoverEvent> ITEM = MapCodecMerger.mapCodec(
                Codec.STRING_IDENTIFIER.converterVerified(verify(TextVerifier_v26_2.class, TextVerifier_v26_2::verifyRegistryItem, "Invalid item")).mapCodec("id").required(), hoverEvent -> hoverEvent.asModern().getId(),
                Codec.rangedInt(1, 99).mapCodec("count").optional().defaulted(1), hoverEvent -> hoverEvent.asModern().getCount(),
                NbtConverter_v26_2.INSTANCE.toCodec().verified(tag -> {
                    if (!tag.isCompoundTag()) return Result.error("Expected a compound tag");
                    return null;
                }).map(NbtTag::asCompoundTag, NbtTag::asCompoundTag).converterVerified(verify(TextVerifier_v26_2.class, TextVerifier_v26_2::verifyDataComponents, "Invalid data components")).mapCodec("components").optional().defaulted(null), hoverEvent -> hoverEvent.asModern().getTag(),
                ItemHoverEvent::new
        );
        public static final MapCodec<EntityHoverEvent> ENTITY = MapCodecMerger.mapCodec(
                Codec.STRING_IDENTIFIER.converterVerified(verify(TextVerifier_v26_2.class, TextVerifier_v26_2::verifyRegistryEntity, "Invalid entity")).mapCodec("id").required(), hoverEvent -> hoverEvent.asModern().getType(),
                ExtraCodecs_v26_2.LENIENT_UUID.mapCodec("uuid").required(), hoverEvent -> hoverEvent.asModern().getUuid(),
                TextCodecs_v26_2.TEXT.mapCodec("name").optional().defaulted(null), hoverEvent -> hoverEvent.asModern().getName(),
