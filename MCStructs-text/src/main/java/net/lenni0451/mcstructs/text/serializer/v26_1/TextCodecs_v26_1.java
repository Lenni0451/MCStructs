package net.lenni0451.mcstructs.text.serializer.v26_1;
...
import net.lenni0451.mcstructs.text.serializer.v1_20_3.TextCodecs_v1_20_3.TextComponentType;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

import static net.lenni0451.mcstructs.text.serializer.v1_20_3.TextCodecs_v1_20_3.createCodec;
import static net.lenni0451.mcstructs.text.serializer.v1_20_3.TextCodecs_v1_20_3.createLegacyComponentMatcher;
import static net.lenni0451.mcstructs.text.serializer.verify.VerifyingConverter.verify;

public class TextCodecs_v26_1 {

    public static final Codec<TextComponent> TEXT = new LazyInitCodec<>(() -> {
        return Codec.recursive(thiz -> createCodec(thiz, StyleCodecs_v26_1.MAP_CODEC, ComponentType.values(), ComponentType::forComponent));
    });
    ...
    public static final MapCodec<TranslationComponent> TRANSLATION_COMPONENT = MapCodecMerger.mapCodec(
            ...
            Codec.either(
                    ...
                    TEXT
            ).map(o -> o instanceof TextComponent ? Either.right((TextComponent) o) : Either.left(o), Either::unwrap).listOf().mapCodec("with").optional().defaulted(List::isEmpty, ArrayList::new), comp -> Arrays.asList(comp.getArgs()),
    ...
    public static final MapCodec<ScoreComponent> SCORE_COMPONENT = MapCodecMerger.codec(
            Codec.STRING.converterVerified(verify(TextVerifier_v26_1.class, TextVerifier_v26_1::verifySelector, "Invalid selector")).mapCodec("name").required(), ScoreComponent::getName,
    ...
    public static final MapCodec<SelectorComponent> SELECTOR_COMPONENT = MapCodecMerger.mapCodec(
            Codec.STRING.converterVerified(verify(TextVerifier_v26_1.class, TextVerifier_v26_1::verifySelector, "Invalid selector")).mapCodec("selector").required(), SelectorComponent::getSelector,
    ...
    public static final MapCodec<NbtComponent> NBT_COMPONENT = MapCodecMerger.mapCodec(
            Codec.STRING.converterVerified(verify(TextVerifier_v26_1.class, TextVerifier_v26_1::verifyNbtPath, "Invalid nbt path")).mapCodec("nbt").required(), NbtComponent::getComponent,
            Codec.BOOLEAN.mapCodec("interpret").optional().lenient().defaulted(false), NbtComponent::isResolve,
            Codec.BOOLEAN.mapCodec("plain").optional().lenient().defaulted(false), NbtComponent::isPlain,
            ...
            NbtComponent::new
    ).verified(comp -> comp.isResolve() && comp.isPlain() ? Result.error("'interpret' and 'plain' flags can't be both on") : null);
    public static final MapCodec<ObjectComponent> OBJECT_COMPONENT = MapCodecMerger.mapCodec(
            ...
            TEXT.mapCodec("fallback").optional().defaulted(null), ObjectComponent::getFallback,
            ObjectComponent::new
    );
    ...
    private enum ComponentType implements TextComponentType {
        ...
        OBJECT("object", OBJECT_COMPONENT);
    ...
    private enum NbtDataSourceType implements NamedType {
        ENTITY("entity", MapCodecMerger.mapCodec(
                Codec.STRING.converterVerified(verify(TextVerifier_v26_1.class, TextVerifier_v26_1::verifySelector, "Invalid selector")).mapCodec("entity").required(), EntityNbtSource::getSelector,
                EntityNbtSource::new
        )),
        BLOCK("block", MapCodecMerger.mapCodec(
                Codec.STRING.converterVerified(verify(TextVerifier_v26_1.class, TextVerifier_v26_1::verifyCoordinates, "Invalid coordinates")).mapCodec("block").required(), BlockNbtSource::getPos,
                BlockNbtSource::new
        )),
