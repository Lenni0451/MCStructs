...
public class TextCodecs_v1_21_5 {
    ...
    private static Codec<TextComponent> createCodec(final Codec<TextComponent> codec) {
        MapCodec<TextComponent> typedComponentCodec = createLegacyComponentMatcher(ComponentType.values(), ComponentType::getCodec, ComponentType::forComponent, "type");
        Codec<TextComponent> styledCodec = MapCodecMerger.codec(
                ...
                StyleCodecs_v1_21_5.MAP_CODEC, TextComponent::getStyle,
        ...
        return Codec.either(Codec.either(Codec.STRING, codec.nonEmptyList()), styledCodec).map(textComponent -> {
            if (textComponent instanceof StringComponent && textComponent.getSiblings().isEmpty() && textComponent.getStyle().isEmpty()) {
                return Either.left(Either.left(((StringComponent) textComponent).getText()));
            } else {
                ...
                stringOrList -> stringOrList.xmap(
                        StringComponent::new,
                        TextCodecs_v1_21_5::mergeComponents
                ),
                second -> second
        ));
    }

    private static <T extends NamedType, E> MapCodec<E> createLegacyComponentMatcher(final T[] types, final Function<T, MapCodec<? extends E>> codecSupplier, final Function<E, T> typeSupplier, final String name) {
    ...
    private enum ComponentType implements NamedType {
        ...
        ComponentType(final String name, final MapCodec<? extends TextComponent> codec) {
            this.name = name;
            this.codec = codec;
        }
    ...
    private enum NbtDataSourceType implements NamedType {
        ...
        NbtDataSourceType(final String name, final MapCodec<? extends NbtDataSource> codec) {
            this.name = name;
            this.codec = codec;
        }
